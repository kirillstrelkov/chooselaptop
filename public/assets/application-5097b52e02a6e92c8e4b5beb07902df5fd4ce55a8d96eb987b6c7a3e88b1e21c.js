...
(function() {
  ...
  $(document).ready(function() {
    ...
    window.cbl.tokenize = function(objOrName) {
      var val;
    ...
    window.cbl.getData = function(rowData) {
      var $rowData, $rowDesc, avgIndex, cpuIndex, cpuModel, desc, gpuModel, gpuPercentage, name, price;
      $rowData = $(rowData);
    ...
    window.cbl.moveToCorrectRow = function(rowElement) {
      var $previousRow, $previousRowDesc, $rowDesc, $rowElement, initialAvgPerc, initialCpuPerc, initialGpuPerc, initialPrice;
      ...
      $previousRow = void 0;
      $('tr.row_data').each(function(index, row) {
        var $row, avgPerc, cpuPerc, gpuPerc, price;
        $row = $(row);
      ...
      if ($previousRow !== void 0 && $previousRow[0] !== $rowElement[0]) {
        $rowElement.detach();
        $rowDesc.detach();
    ...
    window.cbl.fixIndecies = function() {
      var i;
      i = 1;
      return $('table tr').each(function(index) {
        var tds;
        tds = $(this).find('td');
        if (tds.length > 3) {
          $(tds[0]).text(i);
    ...
    window.cbl.updateLaptops = function() {
      var delimiter, new_text;
      $('#laptops').val('');
      ...
      new_text = $('.laptop_desc p').map(function() {
        return $(this).text().trim();
      });
      new_text = new_text.toArray().join(delimiter);
      return $('#laptops').val(new_text);
    };
    window.cbl.getProcessorData = function(modelName, cpusOrGpus) {
      var data, index_name, val;
    ...
    window.cbl.formatPercentage = function(number) {
      return (number * 100).toFixed(2).toString() + '%';
    };
    window.cbl.applyTypeahead = function(inputElement, cpu_or_gpu) {
      var data;
      data = window.cbl[cpu_or_gpu + 's_data'];
      return $(inputElement).typeahead({
        hint: true,
        highlight: true,
        minLength: 1
      }, {
    ...
    window.cbl.updateDesc = function(laptopDataRow, laptopDescRow) {
      var $laptopDataRow, $laptopDescRow, cpu, desc, gpu, price;
      ...
      return $laptopDescRow.find('.fixed_data').text(desc);
    };
    $([window.cbl.CPUS, window.cbl.GPUS]).each(function(index, value) {
      return $.getJSON('/notebookcheck/' + value + '.json', function(resp) {
        ...
        return window.cbl[value.toUpperCase() + '_MAX_INDEX'] = window.cbl[value][window.cbl[value].length - 1].index;
      });
    });
    $('table td.laptop_name a').click(function(event) {
      var $laptop_desc, $parent, $row;
      event.preventDefault();
    ...
    $('form #clear_btn').click(function(event) {
      event.preventDefault();
      $('#laptops').val('');
      $('#delimiter').val('');
    ...
    $('div.checkbox label,div.checkbox input#use_delimiter').click(function(event) {
      if ($('input#use_delimiter').is(':checked')) {
        return $('#delimiter').prop('disabled', false);
      } else {
        return $('#delimiter').prop('disabled', true);
      }
    });
    $('#share_results').click(function(event) {
      var $share_results, delimiter, descriptions, popover, query;
      popover = $('.popover');
      if (popover.length > 0) {
        popover.remove();
      } else {
    ...
    $('table td button.remove').click(function(event) {
      var parent;
      parent = $(this).parent().parent();
      parent.next().remove();
      parent.remove();
      return window.cbl.fixIndecies();
    });
    $('table td button.edit').click(function(event) {
      var $apply, $edit, $input, $price, parent, price;
      parent = $(this).parent().parent();
      $([window.cbl.CPU, window.cbl.GPU]).each(function(index, value) {
        ...
        return window.cbl.applyTypeahead($input, value);
      });
    ...
    $('table td button.apply').click(function(event) {
      var $apply, $avgIndex, $avg_percentage, $cpu_percentage, $edit, $gpu_percentage, $price, avgPercentage, cpuPercentage, fixClass, gpuPercentage, parent, price;
      parent = $(this).parent().parent();
      fixClass = function(element, oldClass, newClass) {
      ...
      $([window.cbl.CPU, window.cbl.GPU]).each(function(index, value) {
        var $a, $aIndex, $index, $model, max_index, model, percentage, processorData, url;
        $model = parent.find('.' + value + '_model');
        ...
        if (model === window.cbl.NOT_FOUND || model.length === 0 || processorData === void 0) {
